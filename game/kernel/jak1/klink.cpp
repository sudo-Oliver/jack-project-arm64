...
uint32_t symlink_v3_arm64_movw(Ptr<uint8_t> link, Ptr<uint8_t> data) {
  ...
  if (strcmp(sym_name, "task-cstage") == 0 || strcmp(sym_name, "method-set!") == 0) {
    fprintf(stderr, "[LINK-DBG] %s arm64_movw: sym_addr=0x%08x s7=0x%08x offset=%d (0x%08x)\n",
  ...
  for (uint32_t i = 0; i < entry_count; i++) {
    // Read each field sequentially via seek to avoid stride misalignment
    // caused by the mode (u8) byte between entries.
    uint32_t movz_offset = *(link + seek).cast<uint32_t>();
    seek += 4;
    uint32_t movk_offset = *(link + seek).cast<uint32_t>();
    seek += 4;
    u8 mode = link.c()[seek];
    seek += 1;
