...
#include "game/kernel/jakx/kmalloc.h"
...
void link_control::jakx_begin(Ptr<uint8_t> object_file,
                              const char* name,
                              int32_t size,
                              Ptr<kheapinfo> heap,
                              uint32_t flags) {
  if (is_opengoal_object(object_file.c())) {
    ...
    m_opengoal = false;
    if (heap == kglobalheap) {
      jakx::kmemopen_from_c(heap, name);
...
uint32_t link_control::jakx_work() {
  ...
  if (m_version == 3) {
    ...
    *((m_link_block_ptr - 4).cast<u32>()) =
        *((s7 + jakx_symbols::FIX_SYM_LINK_BLOCK - 1).cast<u32>());
    rv = jakx_work_opengoal();
  } else if (m_version == 5) {
    ASSERT(!m_opengoal);
    *(u32*)(((u8*)m_link_hdr) - 4) = *((s7 + jakx_symbols::FIX_SYM_LINK_BLOCK - 1).cast<u32>());
    rv = jakx_work_v5();
...
void link_control::jakx_finish(bool jump_from_c_to_goal) {
...
uint64_t link_begin(u64* args) {
  saved_link_control.jakx_begin(Ptr<u8>(args[0]), Ptr<char>(args[1]).c(), args[2],
                                Ptr<kheapinfo>(args[3]), args[4]);
  auto work_result = saved_link_control.jakx_work();
  // if we managed to finish in one shot, take care of calling finish
  if (work_result) {
    // called from goal
    saved_link_control.jakx_finish(false);
  }
...
void ultimate_memcpy(void* dst, void* src, uint32_t size) {
  ...
  if (!(u64(dst) & 0xf) && !(u64(src) & 0xf) && !(u64(size) & 0xf) && size > 0xfff) {
    if (!gfunc_774.offset) {
      ...
      auto sym_val = *((s7 + jakx_symbols::FIX_SYM_ULTIMATE_MEMCPY - 1).cast<u32>());
      if (sym_val == 0) {
...
uint32_t link_control::jakx_work_v2_v4() {
  ...
  if (m_state == LINK_V2_STATE_INIT_COPY) {  // initialization and copying to heap
    ...
    if (m_heap_gap <
      ...
      if (size > OBJ_V2_MAX_TRANSFER) {  // around .5 MB
        jakx::ultimate_memcpy((m_object_data + m_segment_process).c(), source.c(),
                              OBJ_V2_MAX_TRANSFER);
      ...
      if (size) {
        jakx::ultimate_memcpy((m_object_data + m_segment_process).c(), source.c(), size);
    ...
    if (*m_reloc_ptr == 0) {
      ...
      while (true) {
        ...
        if ((relocation & 0x80) == 0) {
          ...
          goalObj = jakx::intern_from_c(-1, 0, name).cast<u8>();
        } else {
          // type!
          ...
          goalObj = jakx::intern_type_from_c(-1, 0, name, nMethods).cast<u8>();
        }
