...
#include "game/graphics/jakx_texture_remap.h"
...
#include "game/kernel/jakx/kmachine_extras.h"
#include "game/kernel/jakx/kmalloc.h"
#include "game/kernel/jakx/kscheme.h"
#include "game/kernel/jakx/ksound.h"
...
int InitMachine() {
  ...
  if (!MasterDebug && !DebugSegment) {
    ...
    kinitheap(kdebugheap, Ptr<u8>(DEBUG_HEAP_START), jakx::DEBUG_HEAP_SIZE);
  }
  init_output();
  InitIOP();
  // sceGsResetPath();
...
void InitMachine_PCPort() {
  ...
  make_function_symbol_from_c("__pc-get-tex-remap", (void*)lookup_jakx_texture_dest_offset);
