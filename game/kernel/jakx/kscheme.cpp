...
#include "game/kernel/jakx/fileio.h"
...
#include "game/kernel/jakx/kprint.h"

#define JAKX_HASH_TABLE
...
Ptr<Function> make_function_from_c_systemv(void* func, bool arg3_is_pp) {
  ...
  *Ptr<u32>(raw.offset) = u32_in_fixed_sym(FIX_SYM_FUNCTION_TYPE);
  ...
  int offset = 0;
  mem.c()[offset++] = 0x48; mem.c()[offset++] = 0xb8;
  for (int i = 0; i < 8; i++) mem.c()[offset++] = target_function[i];
  mem.c()[offset++] = 0x50;
  mem.c()[offset++] = 0x48; mem.c()[offset++] = 0xb8;
  for (int i = 0; i < 8; i++) mem.c()[offset++] = trampoline[i];
  if (arg3_is_pp) { mem.c()[offset++] = 0x4c; mem.c()[offset++] = 0x89; mem.c()[offset++] = 0xe9; }
  mem.c()[offset++] = 0xff; mem.c()[offset++] = 0xe0;
#else
  uint64_t func_addr = (uint64_t)func;
  uint64_t trampoline_addr = (uint64_t)_arg_call_arm64;
  int offset = 0;
  auto write_u32 = [&](uint32_t val) {
    mem.c()[offset++] = val & 0xFF; mem.c()[offset++] = (val >> 8) & 0xFF;
    mem.c()[offset++] = (val >> 16) & 0xFF; mem.c()[offset++] = (val >> 24) & 0xFF;
  };
...
int InitHeapAndSymbol() {
  ...
  *Ptr<u32>(s7.offset + FIX_SYM_EMPTY_CDR - 1) = s7.offset + S7_OFF_FIX_SYM_EMPTY_PAIR;
  ...
  make_stack_arg_function_symbol_from_c("_format", (void*)format_impl_jakx);
