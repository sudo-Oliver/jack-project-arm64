...
/*!
 * @file OceanNearCore.h
 * The near ocean's VU1 emulation. There is no graphics API in it: the water's texture is built by
 * an OceanTextureCore and the vertices go to a CommonOceanRendererCore, both of which a backend
 * subclasses.
 */
...
class OceanNearCore {
 public:
  OceanNearCore(OceanTextureCore& texture, CommonOceanRendererCore& common);

  // `next_bucket` is where this bucket's data ends.
  void render_jak1(DmaFollower& dma, TexturePool* texture_pool, u32 next_bucket);
  void render_jak2(DmaFollower& dma, TexturePool* texture_pool, u32 next_bucket);
 ...
 private:
  ...
  OceanTextureCore& m_texture_renderer;
