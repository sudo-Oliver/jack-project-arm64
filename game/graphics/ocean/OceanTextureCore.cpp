/*!
 * @file OceanTextureCore.cpp
 * See OceanTextureCore.h. Moved here from the OpenGL renderer: the VU1 emulation that builds the
 * ocean's texture, and the DMA that drives it, are unchanged.
 */

#include "OceanTextureCore.h"

#include <cstring>

#include "common/util/Assert.h"

OceanTextureCore::OceanTextureCore(bool generate_mipmaps) : m_generate_mipmaps(generate_mipmaps) {
  m_dbuf_x = m_dbuf_a;
  m_dbuf_y = m_dbuf_b;
  m_tbuf_x = m_tbuf_a;
  m_tbuf_y = m_tbuf_b;
...
OceanTextureCore::~OceanTextureCore() = default;

void OceanTextureCore::run_L1_PC() {
  //  L1:
...
void OceanTextureCore::run_L2_PC() {
  //  L2:
  //  xtop vi05_in_ptr
  vu.in_ptr = swap_vu_upload_buffers();
  //  mtir vi06_dbuf_write, vf04_dbuf.x
...
void OceanTextureCore::run_L3_PC() {
  Vf base_pos;  // vf15
  u16 loop_idx;
...
void OceanTextureCore::run_L5_PC() {
  // clang-format off
  u16 loop_idx;
  Vf res0;
  Vf res1;
...
void OceanTextureCore::xgkick_PC(Vf* src) {
...
void OceanTextureCore::run_L1_PC_jak2() {
  //  L1:
  //  lq.xyzw vf14_startx, 988(vi00)    |  maxw.xyzw vf01_ones, vf00, vf00
  vu.startx = Vf(m_texture_constants.start);
  //  lq.xyzw vf02_offset, 989(vi00)
...
void OceanTextureCore::run_L2_PC_jak2() {
  //  L2:
  //  xtop vi05_in_ptr
  vu.in_ptr = swap_vu_upload_buffers();
  //  mtir vi06_dbuf_write, vf04_dbuf.x
...
void OceanTextureCore::run_L3_PC_jak2() {
  Vf base_pos;  // vf15
  u16 loop_idx;
...
void OceanTextureCore::handle_ocean_texture_jak1(DmaFollower& dma, TexturePool* texture_pool) {
  m_texture_pool = texture_pool;
  // With mipmaps the grid goes into the scratch target and the chain is built from it; without,
  // it goes straight into the result.
  backend_begin_pass(m_generate_mipmaps);
  ...
  if (m_generate_mipmaps) {
    backend_make_texture_with_mipmaps();
  }

  m_texture_pool->move_existing_to_vram(m_tex0_gpu, OCEAN_TEX_TBP_JAK1);
}


void OceanTextureCore::handle_ocean_texture_jak2(DmaFollower& dma, TexturePool* texture_pool) {
  m_texture_pool = texture_pool;
  backend_begin_pass(m_generate_mipmaps);
  // render to the first texture
  {
  ...
  // give to gpu!
  m_texture_pool->move_existing_to_vram(m_tex0_gpu, OCEAN_TEX_TBP_JAK2);
}
