...
/*!
 * @file ShadowRendererCore.h
 * The shadow renderer with no graphics API in it.
 *
 * The shadow bucket draws the character shadows as stencil shadow volumes: the game sends a
 * shadow mesh, a VU program extrudes it away from the light, and the renderer draws the front and
 * back faces of that volume into the stencil buffer to find the pixels inside it.
 *
 * Everything up to "draw the volume" is VU1 emulation and has no graphics API in it, so it lives
 * here. A backend implements draw_volumes(), which is the stencil work.
 */
...
class ShadowRendererCore {
 ...
 protected:
  ...
  struct Context {
    ...
    u32 next_bucket = 0;
    // A disabled bucket still walks its DMA, it just draws nothing.
    bool enabled = true;
  };

  // Walks one bucket's DMA, runs the VU program, and calls draw_volumes() with the result.
  void render_core(DmaFollower& dma, const Context& context);

  // Draw m_next_front_index front-facing and m_next_back_index back-facing indices out of
  // m_vertices into the stencil buffer, then shade the pixels the two passes disagree on with
  // m_color. The core has already appended the full-screen quad the last step draws: it is the
  // six indices at the end of m_front_indices.
  virtual void draw_volumes() = 0;
