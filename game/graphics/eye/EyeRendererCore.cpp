/*!
 * @file EyeRendererCore.cpp
 * See EyeRendererCore.h. Moved here from the OpenGL renderer: the DMA decode that works out which
 * quads an eye is made of, and the vertex buffer it fills, are unchanged.
 */

#include "EyeRendererCore.h"
...
EyeRendererCore::EyeRendererCore() = default;
EyeRendererCore::~EyeRendererCore() = default;

void EyeRendererCore::init_textures(TexturePool& texture_pool, GameVersion version) {
  // set up eyes
  for (int pair_idx = 0; pair_idx < NUM_EYE_PAIRS; pair_idx++) {
    for (int lr = 0; lr < 2; lr++) {
      ...
      TextureInput in;
      m_gpu_eye_textures[tidx].handle = backend_create_eye_texture(tidx);
...
void EyeRendererCore::render_core(DmaFollower& dma,
  ...
  handle_eye_dma2(dma, texture_pool, version);
...
EyeRendererCore::ScissorInfo decode_scissor(const DmaTransfer& dma) {
  ASSERT(dma.vif0() == 0);
  ...
  EyeRendererCore::ScissorInfo result;
  u64 val;
...
EyeRendererCore::SpriteInfo decode_sprite(const DmaTransfer& dma) {
  /*
  ...
  EyeRendererCore::SpriteInfo result;
...
EyeRendererCore::EyeDraw read_eye_draw(DmaFollower& dma) {
...
std::vector<EyeRendererCore::SingleEyeDraws> EyeRendererCore::get_draws(DmaFollower& dma,
                                                                       TexturePool* texture_pool,
                                                                       GameVersion version) {
  ...
  while (dma.current_tag().qwc != 8) {
    ...
    auto tex0 = texture_pool->lookup_gpu_texture(adgif0.tex0().tbp0());
    ...
    // up next is the pupil background
    {
      ...
      l_draw.iris_gl_tex = *texture_pool->lookup(adgif0.tex0().tbp0());
      ...
      if (dma.current_tag().qwc == 6) {
        ...
        r_draw.iris_tex =
            texture_pool->lookup_gpu_texture(r_iris_helper.tex0().tbp0());
        r_draw.iris_gl_tex = *texture_pool->lookup(r_iris_helper.tex0().tbp0());
      } else {
        // same adgif
        r_draw.iris = read_eye_draw(dma);
    ...
    auto tex1 = texture_pool->lookup_gpu_texture(adgif1.tex0().tbp0());

    if (tex1 && tex1->get_data_ptr()) {
      l_draw.pupil = read_eye_draw(dma);
      l_draw.pupil_tex = tex1;
      l_draw.pupil_gl_tex = *texture_pool->lookup(adgif1.tex0().tbp0());
    }
    ...
    if (dma.current_tag().qwc == 6) {
      ...
      r_draw.pupil_tex =
          texture_pool->lookup_gpu_texture(r_pupil_helper.tex0().tbp0());
      r_draw.pupil_gl_tex = *texture_pool->lookup(r_pupil_helper.tex0().tbp0());
    } else {
      if (tex1 && tex1->get_data_ptr()) {
        r_draw.pupil = read_eye_draw(dma);
    ...
    auto tex2 = texture_pool->lookup_gpu_texture(adgif2.tex0().tbp0());

    {
      l_draw.lid = read_eye_draw(dma);
      l_draw.lid_tex = tex2;
      l_draw.lid_gl_tex = *texture_pool->lookup(adgif2.tex0().tbp0());
    }
    ...
    if (dma.current_tag().qwc == 6) {
      ...
      r_draw.lid_tex = texture_pool->lookup_gpu_texture(r_lid_helper.tex0().tbp0());
      r_draw.lid_gl_tex = *texture_pool->lookup(r_lid_helper.tex0().tbp0());
    } else {
      r_draw.lid = read_eye_draw(dma);
      r_draw.lid_tex = tex2;
...
void EyeRendererCore::handle_eye_dma2(DmaFollower& dma,
                                      TexturePool* texture_pool,
                                      GameVersion version) {
  m_texture_pool = texture_pool;
  Timer timer;
  ...
  auto draws = get_draws(dma, texture_pool, version);
  backend_run_gpu(draws);
...
std::optional<u64> EyeRendererCore::lookup_eye_texture(u8 eye_id) {
  eye_id = (eye_id % 40);
...
std::optional<u64> EyeRendererCore::lookup_eye_texture_hash(u64 hash, bool lr) {
...
std::string EyeRendererCore::SpriteInfo::print() const {
  std::string result;
...
std::string EyeRendererCore::ScissorInfo::print() const {
  return fmt::format("x : [{}, {}], y : [{}, {}]", x0, x1, y0, y1);
}

std::string EyeRendererCore::EyeDraw::print() const {
