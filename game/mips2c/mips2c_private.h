...
inline void mips2c_watch_spad_meta_write(const char* op,
                                         u32 ee_addr,
                                         const void* src,
                                         size_t size,
                                         void* ret_addr) {
  constexpr u32 watch_start = 0x1d0000 + 6752;
  constexpr u32 watch_end = 0x1d0000 + 7352;
  if (ee_addr < watch_end && ee_addr + size > watch_start) {
    u32 val = 0;
    memcpy(&val, src, size < sizeof(val) ? size : sizeof(val));
    fmt::print(stderr, "[SPAD-META-WR] op={} addr={:08x} size={} val={:08x} ret={}\n", op,
               ee_addr, size, val, fmt::ptr(ret_addr));
  }
}
...
struct ExecutionContext {
  ...
  void jalr(u32 addr) {
    ...
    if (!addr) {
      static u32 s_null_jalr_count = 0;
      if (s_null_jalr_count < 64) {
        fmt::print(stderr,
                   "[MIPS2C-JALR0] caller={} a0={:08x} a1={:08x} a2={:08x} a3={:08x} "
                   "t0={:08x} t1={:08x} t2={:08x} t3={:08x} pp={:08x} st={:08x}\n",
                   fmt::ptr(__builtin_return_address(0)), (u32)args[0], (u32)args[1],
                   (u32)args[2], (u32)args[3], (u32)args[4], (u32)args[5], (u32)args[6],
                   (u32)args[7], gprs[s6].du32[0], gprs[s7].du32[0]);
      }
      s_null_jalr_count++;
      gprs[v0].du64[0] = 0;
      return;
    ...
    gprs[v0].du64[0] =
        _call_goal8_asm_arm64(g_ee_main_mem + addr, args, 0, gprs[s6].du64[0],
                              gprs[s7].du64[0], g_ee_main_mem);
    // Keep exec mode after nested GOAL calls. MIPS2C may be called from GOAL and return to GOAL;
    // write mode here causes a MAP_JIT instruction-fetch fault at the outer return site.
#elif _WIN32
  ...
  void sq(int src, int offset, int addr) {
    auto s = gpr_src(src);
    // ASSERT((offset & 15) == 0);
    const u32 ee_addr = (gpr_addr(addr) + offset) & (~15u);
    memcpy(g_ee_main_mem + ee_addr, &s.du32[0], 16);
  }
